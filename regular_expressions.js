...
function has_ktp(string) {
  let result = (/[0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9][0-9][0-9]$/).test(string);
  return result
}
...
function grab_ktp(string) {
  let index = string.search(/[0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9][0-9][0-9]$/)
  if (index != -1){
    let result = string.substr(index,11)
    return result;
  } else {
    return null;
  }
