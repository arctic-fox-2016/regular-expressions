...
function has_ktp(string) {
  ...
  // console.log(string.search(/[1-9][0-9][0-9]\-[1-9][0-9]\-[1-9][0-9][0-9][0-9]/));
}
...
function grab_ktp(string) {
  var ktp = string.substr(string.search(re), 11);
...
function grab_all_nomor_ktp(string) {
  if (string.match(re) == null) {
    ...
    return string.match(re).input.replace(/ /g, "").split(",");
  }
}
...
function hide_all_nomor_ktp(string) {
  if (string.match(re) == null) {
    ...
    return string.match(re).input.replace(/[0-9][0-9][0-9]\-[0-9][0-9]\-/g, "XXX-XX-").replace(/ /g, "").split(",");
  }
}
...
function format_nomor(string) {
  return string.replace(/\./g, "").replace(/\-/g, "").replace(/(\d\d\d)(\d\d)(\d\d\d\d)/g, "$1-$2-$3");
}
